# STAGE 047 — Implement negative CLI scenarios
...
- `tests/cli/**`
- `src/cli/**` only for testability wiring when the repo keeps CLI tests in
  the in-file test module and no separate `tests/cli/**` harness exists
...
1. Add negative tests: missing workspace, bad node type, bad status, duplicate id, broken link, unsafe tool run, deprecated recall exclusion
...
5. Write `.devplan/HANDOFFS/STAGE_047.md`.
